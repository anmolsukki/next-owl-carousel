...
class OwlCarousel extends React.Component {
	...
	getOptions() {
		...
		let carOptions = Object.values(owlCarouselOptions).reduce((a, v) => a.concat(v), []);
		carOptions.forEach((val) => {
			if (val in this.props.options) options[val] = this.props.options[val];
		});

		let carEvents = Object.values(owlCarouselEvents).reduce((a, v) => a.concat(v), []);
		carEvents.forEach((val) => {
            if(this.props.events) {
				if (val in this.props.events) options[val] = this.props.events[val];
            }
		});
		return options;
	}
...
OwlCarousel.propTypes = {
	...
	options: PropTypes.shape({
		...
		// animate
		animateOut: PropTypes.oneOfType([PropTypes.bool, PropTypes.string]),
		animateIn: PropTypes.oneOfType([PropTypes.bool, PropTypes.string]),

		// autoplay
		autoplay: PropTypes.bool,
		autoplayTimeout: PropTypes.number,
		autoplayHoverPause: PropTypes.bool,
		autoplaySpeed: PropTypes.oneOfType([PropTypes.bool, PropTypes.number]),
